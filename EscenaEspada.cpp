...
EspadaScene::EspadaScene(sf::RenderWindow& windowRef, int& vidasJugador,
  ...
  // Piedra
  spritePiedra = new sf::Sprite(texPiedra);
  spritePiedra->setOrigin({texPiedra.getSize().x / 2.f, texPiedra.getSize().y / 2.f});
  spritePiedra->setPosition(POS_PIEDRA);

  // Espada
  spriteEspada = new sf::Sprite(texEspada);
  spriteEspada->setOrigin({texEspada.getSize().x / 2.f, texEspada.getSize().y * 0.8f});
...
void EspadaScene::inicializarPreguntas() {
  ...
  colaPreguntas.push(Pregunta("Durante el renacimiento, el modelo de gobierno es uno de los siguientes:", {"A) Monarquía absoluta", "B) Tiranía republicana", "C) Democracia participativa", "D) Liberalismo político"}, 'A'));
  colaPreguntas.push(Pregunta("De los siguientes acontecimientos, seleccione el que inicia el período moderno:", {"A) Toma de Constantinopla", "B) Tratado de Paz de Westfalia", "C) Toma de la Bastilla", "D) La ruta de la seda"}, 'B'));
  colaPreguntas.push(Pregunta("Durante el siglo XV, la sociedad se estratifica en tres estamentos definidos:", {"A) Clase media, baja y alta", "B) Nobleza, clero y estado llano", "C) Artesanos, guardianes y gobernantes", "D) Ninguna de las anteriores"}, 'B'));
  colaPreguntas.push(Pregunta("Aparece el realismo político, que se basaba en un orden establecido y recomendaciones de cómo gobernar:", {"A) Tomás Moro", "B) Jean Bodín", "C) Nicolás Maquiavelo", "D) Erasmo de Rotterdam"}, 'C'));
  colaPreguntas.push(Pregunta("Terminada la edad media, en el contexto de la política resulta que:", {"A) La Iglesia resalta su poder", "B) La Iglesia pierde el papel rector en la política", "C) La Iglesia evangélica se posiciona en la política", "D) La política desaparece"}, 'B'));
}
// clang-format on
...
void EspadaScene::procesarRespuesta(char tecla) {
  ...
  if (correcta) {
    ...
    if (respuestasCorrectas >= totalPreguntasIniciales) {
      gano = true;
      vidasJugador++;
    };
  } else {
...
void EspadaScene::resetEspada() {
  espadaY = 0.f;
  animando = false;
  while (!colaPreguntas.empty()) colaPreguntas.pop();
...
void EspadaScene::draw(sf::RenderWindow& window) {
  ...
  if (!terminado && preguntaActualPtr) {
    ...
    resultado.setString(gano ? "¡Has sacado la espada! Presiona Enter para regresar a La Ruleta del Saber"
                             : "No lograste sacar la espada! Presiona Enter para regresar a La Ruleta del Saber");
    resultado.setPosition({100.f, 650.f});
    window.draw(resultado);
  }
}

bool EspadaScene::wantsToChangeScene() const {
  return terminado && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Enter);
}

Scene* EspadaScene::nextScene(sf::RenderWindow& window, int& vidasJugador) {
